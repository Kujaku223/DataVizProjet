  ...
  private createChart(selectedContinents: string[]) {
    ...
    countryLines
      ...
      .on('mousemove', (event, [countryName, happinessRecords]) => {
        ...
        d3.selectAll('#countryPath').style('stroke-width', 1.5); // fix bug where mousemove too fast doesn't remove previous stroke-width
      ...
      .on('mouseout', (event, d) => {
        ...
        d3.selectAll('#countryPath').style('opacity', 0.5);
